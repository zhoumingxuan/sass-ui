...
function Item({
  ...
  const depthBg = depth > 0 ? 'bg-nav-sub' : '';
  const hoverBg = depth > 0 ? 'hover:bg-nav-sub-hover' : 'hover:bg-nav-hover';
  const openClass = hasChildren && open ? 'bg-nav-hover text-white' : '';
  return (
    <div>
      <div
        className={`flex items-center gap-2 p-2 rounded-md cursor-pointer transition-colors text-gray-200 ${depthBg} ${hoverBg} hover:text-white ${openClass} ${
...
export default function Menu({
  ...
  return (
    <aside
      className={`flex flex-col h-screen border-r border-nav-hover shadow-lg bg-gradient-to-b from-nav-deep to-nav text-gray-200 transition-all ${
        collapsed ? 'w-16' : 'w-56'
      }`}
    >
      ...
      <div className="p-2 space-y-1 border-t border-nav-hover">
